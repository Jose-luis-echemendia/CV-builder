...
class Settings(BaseSettings):
    ...
    def all_cors_origins(self) -> list[str]:
        origins = getattr(self, "BACKEND_CORS_ORIGINS", []) or []
        frontend_host = getattr(self, "FRONTEND_HOST", None)
        normalized = [str(origin).rstrip("/") for origin in origins]
        if frontend_host:
            normalized.append(frontend_host)
        return normalized
    ...
    MINIO_ENDPOINT: str = Field(
        ...
        description="Host:puerto de MinIO sin esquema",
    )
    ...
    # ── PDF ───────────────────────────────────────────────────────────────────
    PDF_TEMPLATES_DIR: Path = TEMPLATES_DIR / "cv"
    PDF_DPI: int = Field(default=150, ge=72, le=300)

    # Limpieza automática de PDFs expirados (Celery Beat)
    PDF_EXPIRY_HOURS: int = Field(
        default=24,
        ge=1,
        description="Horas tras las cuales el PDF se considera expirado y se elimina de MinIO",
    )
