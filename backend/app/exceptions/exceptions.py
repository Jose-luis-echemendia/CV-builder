...
def _build_json_response(
    ...
    if http_code >= 500:
        ...
        logger.warning(
            f"ID: {request_id} | Error {http_code} en {request.url.path} | {description}"
        )
...
def _extract_object_from_request(request: Request) -> str:
    """Extrae el recurso y la operación (ej: resource.get) del path."""
...
class PermissionDenied(ForbiddenException):
    """Excepción lanzada cuando la acción está prohibida para el contexto actual.

    HTTP Status: 403 Forbidden

    Uso cuando:
    - Se intenta realizar una acción para la que no hay permisos
