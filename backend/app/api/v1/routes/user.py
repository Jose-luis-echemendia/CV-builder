...
# ===========================================================================
#           --- Endpoint para listar Usuarios solo ADMIN o DEVELOPER ---
# ===========================================================================
...
async def read_users(
    *,
    ...
    users = await user_repo.get_all(skip=skip, limit=limit, active_only=is_active)
    count = await user_repo.count(active_only=is_active)
...
async def read_users_by_role(
    *,
    ...
    users = await user_repo.search(role, skip=skip, limit=limit, active_only=is_active)
    # Para búsqueda, necesitamos contar todos los resultados que coinciden
    all_users = await user_repo.search(role, skip=0, limit=10000, active_only=is_active)
    count = len(all_users)
    ...
    if user_in.role == UserRole.DEVELOPER:
        ...
        if current_user.role != UserRole.DEVELOPER:
            ...
            raise PermissionDenied(
                "Solo usuarios con rol DEVELOPER pueden crear otros desarrolladores"
            )
        ...
        raise PermissionDenied("No tienes permisos suficientes para crear usuarios")
...
async def delete_user_me(
    *, user_repo: AsyncUserRepoDep, current_user: CurrentUser
    ...
    if current_user.is_superuser:
        ...
        raise PermissionDenied("Super users are not allowed to delete themselves")
...
async def read_user_by_id(
    *, user_id: uuid.UUID, user_repo: AsyncUserRepoDep, current_user: CurrentUser
    ...
    if not current_user.is_superuser:
        ...
        raise PermissionDenied("The user doesn't have enough privileges")
...
async def update_user(
    *,
    ...
    if not db_user:
        ...
        raise UserNotFound("The user with this id does not exist in the system")
...
async def search_users(
    *,
    ...
    users = await user_repo.search(
        search_term=search_term, skip=skip, limit=limit, active_only=is_active
    )
    count = await user_repo.count_search(search_term, active_only=is_active)
...
async def activate_user(
    *, user_id: uuid.UUID, user_repo: AsyncUserRepoDep, current_user: CurrentUser
    ...
    if getattr(user, "is_active", False):
        logger.info(f"ℹ️ Usuario {user.email} ya está activo")
        raise UserActive("User is already active")
...
async def deactivate_user(
    *, user_id: uuid.UUID, user_repo: AsyncUserRepoDep, current_user: CurrentUser
    ...
    if not getattr(user, "is_active", True):
        logger.info(f"ℹ️ Usuario {user.email} ya está inactivo")
        raise UserInactive("User is already inactive")

    if user == current_user:
        logger.warning(
            f"❌ Superusuario {current_user.email} intentó auto-desactivarse"
        )
        raise PermissionDenied("Cannot deactivate yourself")
