...
class RedisService:
    ...
    async def init_async(cls) -> None:
        ...
        """
        logger.debug("Iniciando conexión Redis (async) a %s", settings.REDIS_URL_CACHE)
        try:
            cls._async_client = aioredis.from_url(
                settings.REDIS_URL_CACHE, encoding="utf-8", decode_responses=True
            )
            pong = await cls._async_client.ping()
            if pong:
                FastAPICache.init(
                    RedisBackend(cls._async_client), prefix="fastapi-cache"
                )
                logger.info(
                    "RedisService (async) conectado y FastAPICache inicializado."
                )
            else:
                logger.warning(
                    "RedisService (async) conectado pero ping devolvió falsy: %s", pong
                )
    ...
    def init_sync(cls) -> None:
        ...
        """
        logger.debug("Iniciando conexión Redis (sync) a %s", settings.REDIS_URL_CACHE)
        try:
            cls._sync_client = redis.Redis.from_url(
                settings.REDIS_URL_CACHE, decode_responses=True
            )
            ...
            if pong:
                ...
                logger.warning(
                    "RedisService (sync) conectado pero ping devolvió falsy: %s", pong
                )
    ...
    def get_sync(cls) -> redis.Redis | None:
        ...
        logger.debug(
            "Obteniendo cliente Redis (sync): %s",
            "disponible" if cls._sync_client else "None",
        )
    ...
    def get_async(cls) -> aioredis.Redis | None:
        ...
        logger.debug(
            "Obteniendo cliente Redis (async): %s",
            "disponible" if cls._async_client else "None",
        )
