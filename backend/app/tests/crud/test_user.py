...
def test_authenticate_user(db: Session) -> None:
    ...
    async def _create_and_auth():
        async with AsyncSession(a_engine) as session:
            ...
            authenticated_user = await repo.authenticate(email=email, password=password)
            return user, authenticated_user

    user, authenticated_user = asyncio.run(_create_and_auth())
...
def test_not_authenticate_user(db: Session) -> None:
    ...
    async def _auth():
        async with AsyncSession(a_engine) as session:
            repo = UserRepositoryAsync(session=session)
            return await repo.authenticate(email=email, password=password)

    user = asyncio.run(_auth())
    assert user is None
...
def test_update_user(db: Session) -> None:
    ...
    user, new_password = asyncio.run(_create_and_update())
    user_2 = db.get(User, user.id)
    assert user_2
    assert user.email == user_2.email
